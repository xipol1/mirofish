...
// Lazy-load benchmark files once.
let _benchCache = null;
function loadBenchmarks() {
  if (_benchCache) return _benchCache;
  const benchDir = path.join(__dirname, '..', '..', 'data', 'benchmarks');
  const elasticity = JSON.parse(fs.readFileSync(path.join(benchDir, 'hotel_elasticity_benchmarks.json'), 'utf-8'));
  const egatur = JSON.parse(fs.readFileSync(path.join(benchDir, 'egatur_2024_spending.json'), 'utf-8'));
  _benchCache = { elasticity, egatur };
  return _benchCache;
}
